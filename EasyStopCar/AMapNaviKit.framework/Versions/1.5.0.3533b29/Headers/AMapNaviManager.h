...
/*!
 @brief 前方拥堵时是否重新计算路径，默认YES（只适用于驾车导航，需要联网）
 */
...
/*!
 @brief 是否采用外部传入定位信息，默认NO
 */
@property (nonatomic, assign) BOOL enableExternalLocation;

/*!
 @brief 外部传入定位信息(enableExternalLocation为YES时有效)
 */
...
/*!
 @brief 获取导航统计信息。(只适用于实时导航)
 @return 航统计信息
 */
- (AMapNaviStatisticsInfo *)getNaviStatisticsInfo;
...
/*!
 @brief 驾车导航时，前方遇到拥堵需要重新计算路径时的回调函数
 */
- (void)naviManagerNeedRecalculateRouteForTrafficJam:(AMapNaviManager *)naviManager;
...
/*!
 @brief 需要显示路口放大图时的回调函数
 @param crossImage 路口放大图Image(size:400*400)
 */
- (void)naviManager:(AMapNaviManager *)naviManager onShowCrossImage:(UIImage *)crossImage;

/*!
 @brief 需要隐藏路口放大图时的回调函数
 */
- (void)naviManagerOnHideCrossImage:(AMapNaviManager *)naviManager;

/*!
 @brief 需要显示车道信息时的回调函数
        可通过 UIImage *CreateLaneInfoImageWithLaneInfo(NSString *laneBackInfo, NSString *laneSelectInfo); 方法创建车道信息图片
 @brief 0-直行; 1-左转; 2-直行和左转; 3-右转;
 ...
 */
- (void)naviManager:(AMapNaviManager *)naviManager onShowLaneBackInfo:(NSString *)laneBackInfo laneSelectInfo:(NSString *)laneSelectInfo;

/*!
 @brief 需要隐藏车道信息时的回调函数
 */
- (void)naviManagerOnHideLaneInfo:(AMapNaviManager *)naviManager;
